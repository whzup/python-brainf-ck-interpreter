...
class Parser:
    ...
    def match(self, exp):
        ...
        if self.lookahead.value == EOF_VALUE:
            self.logger.debug("Matched EOF")
            return 0
        elif self.lookahead.value == exp:
            self.consume()
            self.logger.debug("Parser matched {}".format(exp))
        else:
            self.logger.error("Expected {}, found {}".format(exp, self.lookahead.value))
    ...
    def operation(self):
        ...
        if self.lookahead.value in OPERATIONS:
            ...
            self.logger.error("Expected operation or loop start, found {}".format(self.lookahead.value))
